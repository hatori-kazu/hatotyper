...
class ScreenCaptureService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            startForeground(200, notification, ServiceInfo.FOREGROUND_SERVICE_TYPE_MEDIA_PROJECTION)
        } else {
        ...
        if (mediaProjection == null) {
            ...
            mediaProjection?.registerCallback(object : MediaProjection.Callback() {
                override fun onStop() {
                    LogManager.appendLog(TAG, "停止: システムにより終了")
                    stopSelf()
                }
                ...
                if (cropW > 0 && cropH > 0) {
                    ...
                    recognizer.process(inputImage)
                        ...
                        .addOnFailureListener { e ->
                            LogManager.appendLog(TAG, "OCR失敗: ${e.localizedMessage}")
                        }
    ...
    private fun processDetectedText(rawText: String) {
        ...
        val matchedTriggers = allMappings.filter { it.isEnabled && cleanedText.contains(
import android.app.NotificationChannel
import android.app.NotificationManager
import android.app.Service
import android.content.Context
